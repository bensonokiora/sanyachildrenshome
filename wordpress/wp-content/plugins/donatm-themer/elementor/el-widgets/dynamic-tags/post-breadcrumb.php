...
class GVAElement_Post_Breadcrumb extends GVAElement_Base{
	...
	public function get_title(){
		return esc_html__('Breadcrumb', 'donatm-themer');
	}
	...
	protected function register_controls(){
		...
		$this->add_control(
			'bg_overlay_color',
			[
				'label' => esc_html__( 'Background Overlay Color', 'donatm-themer' ),
		...
		$this->add_control(
	  	...
	  	$this->add_responsive_control(
			'padding_top',
			[
				'label' => __( 'Breadcrumb Padding Top', 'donatm-themer' ),
		...
		$this->add_responsive_control(
			'padding_bottom',
			[
				'label' => __( 'Breadcrumb Padding Bottom', 'donatm-themer' ),
				'type' => Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'min' => 0,
		...
		$this->add_control(
			'heading_content',
			[
				'label'     => __('Content & Style ------', 'donatm-themer'),
				'type'      => Controls_Manager::HEADING,
			]
	  	);
		$this->add_control(
			'show_title',
			[
				'label'     => __('Show Titlte', 'donatm-themer'),
		...
		$this->add_control(
			'title_color',
			[
				'label' => esc_html__( 'Title Color', 'donatm-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .donatm-post-tags .title',
			]
		);
		...
		$this->add_control(
			'show_links',
			[
				'label'     => __('Show Breadcrumb Links', 'donatm-themer'),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'yes',
	  	...
	  	$this->add_control(
			'breadcrumb_link_bg',
			[
				'label' => esc_html__( 'Background Color Breadcrumb Links', 'donatm-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
	...
	public function breadcrumbs(){
	 	$delimiter = '';
	 	$home = esc_html__('Home', 'donatm-themer');
	 	...
	 	if(!is_home() && !is_front_page() || is_paged()) {
			...
			if(is_category()){
			  	...
			  	$breadcrumb .= $before . esc_html__('Search results for', 'donatm-themer') . '"' . get_search_query() . '"' . $after;
			  ...
			  $breadcrumb .= $before . esc_html__('Posts tagged', 'donatm-themer') . '"' . single_tag_title('', false) . '"' . $after;
			  	...
			  	if($userdata){
				 	$breadcrumb .= $before . esc_html__('Articles posted by', 'donatm-themer') . $userdata->display_name . $after;
			  	} 

			}elseif(is_404()){
			  	$breadcrumb .= $before . esc_html__('Error 404', 'donatm-themer') . $after;
			}

			if(get_query_var('paged')){
			  	$breadcrumb .= $before . esc_html__('Page','donatm-themer') . ' ' . get_query_var('paged') . $after;
			}
