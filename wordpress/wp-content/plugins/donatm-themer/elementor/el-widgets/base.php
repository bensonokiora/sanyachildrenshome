...
abstract class GVAElement_Base extends Elementor\Widget_Base {
   ...
   protected function render(){
      global $donatm_post, $post, $donatm_term_id, $donatm_job_preview;
      ...
      if ($post_type === 'gva__template' || $post_type === 'elementor_library'){
      	...
      	$preview_post_id = 0;
      	$donatm_term_id = 0;
      	$post_preview = $document->get_settings('donatm_post_preview');
      	...
      	switch ($template_type){
      		...
      		if ($terms) {
      			$term = current($terms);
      			$donatm_term_id = $term->term_id;
      		}
      	}
         $donatm_post = get_post($preview_post_id);
      }else{
      	...
      	if($_submit_form_page == $post_id && isset($_POST['job_manager_form']) && isset($_POST['submit_job']) && $_POST['job_manager_form'] == 'submit-job' && $_POST['submit_job'] ==  __( 'Preview', 'wp-job-manager' )){
				$donatm_post = get_post($donatm_job_preview);
			}else{
      		$donatm_post = $post;
      	}
         ...
         if(!empty($object)){
            $donatm_term_id = isset($object->term_id) && $object->term_id ? $object->term_id : 0;
         }
      }
   }

   protected function add_control_image_size($default, $key = 'donatm_image', $label = ''){
        if (empty($label)) {
            $label = esc_html__('Image Thumbnail', 'donatm-themer');
        }
  ...
  protected function add_control_carousel($columns_default, $condition = array()) {
	  	$this->start_controls_section(
			...
			[
				'label' => __('Carousel Options', 'donatm-themer'),
	  	...
	  	if($columns_default != 'always_single'){
		 	$this->add_control(
			 	'ca_items_lg',
			 	[
					'label'     => __('Columns for Large Screen', 'donatm-themer'),
		  	...
		  	$this->add_control(
			 	'ca_items_md',
			 	[
					'label'     => __('Columns for Medium Screen', 'donatm-themer'),
					'type'      => Controls_Manager::SELECT,
			...
			$this->add_control(
				'ca_items_sm',
				[
				  	'label'     => __('Columns for Small Screen', 'donatm-themer'),
				  	'type'      => Controls_Manager::SELECT,
			...
			$this->add_control(
				'ca_items_xs',
				[
				  'label'     => __('Columns for Extra Small Screen', 'donatm-themer'),
				  'type'      => Controls_Manager::SELECT,
			...
			$this->add_control(
				'ca_items_xx',
				[
				  'label'     => __('Columns for Very Extra Small Screen', 'donatm-themer'),
				  'type'      => Controls_Manager::SELECT,
				  'default'   => 1,
		...
		$this->add_control(
			'ca_effect',
			[
				'label'     => __('Effect', 'donatm-themer'),
				'type'      => Controls_Manager::SELECT,
				'default'   => 'slide',
				'options'   => array(
					'slide'		=>	__('Slide', 'donatm-themer'),
					'coverflow'	=>	__('coverflow', 'donatm-themer'),
				)
			]
		);
		$this->add_control(
		...
		$this->add_control(
		 	'ca_pagination_type',
		 	[
				'label'     => __('Pagination Type', 'donatm-themer'),
				'type'      => Controls_Manager::SELECT,
				'default'   => 'bullets',
				'options'   => array(
					'bullets'		=> esc_html__( 'Bullets', 'donatm-themer'),
					'fraction'		=> esc_html__( 'Fraction', 'donatm-themer'),
					'progressbar'	=> esc_html__( 'Progressbar', 'donatm-themer')
				)
			]
		);
		...
		$this->add_control(
			'ca_dynamic_bullets',
			[
				'label'     => __('Dynamic Bullets', 'donatm-themer'),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'no'
			]
		);
		...
		$this->add_responsive_control(
		  'spacing_dots',
		  	[
			 	'label' => __( 'Dots Spacing', 'donatm-themer' ),
	 ...
	 protected function add_control_grid($condition = array()) {
	  $this->start_controls_section(
		  ...
		  [
			 'label' => __('Grid Options', 'donatm-themer'),
			 'type'  => Controls_Manager::SECTION,
			 'condition' => $condition,
		  ]
	  );
	  ...
	  $this->add_control(
		...
		$this->add_control(
		  ...
		  $this->add_control(
			 'grid_remove_padding',
			 [
				'label'     => __('Remove Padding', 'donatm-themer'),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'no',
			 ]
		  );
   ...
   protected function get_carousel_options($options){
      $carousel_options = array(
        'items'               => isset($options['ca_items_lg']) ? intval($options['ca_items_lg']) : 1,
        'items_lg'            => isset($options['ca_items_lg']) ? intval($options['ca_items_lg']) : 1,
        'items_md'            => isset($options['ca_items_md']) ? intval($options['ca_items_md']) : 1,
        'items_sm'            => isset($options['ca_items_sm']) ? intval($options['ca_items_sm']) : 1,
        'items_xs'            => isset($options['ca_items_xs']) ? intval($options['ca_items_xs']) : 1,
        'items_xx'            => isset($options['ca_items_xx']) ? intval($options['ca_items_xx']) : 1,
        'effect'              => isset($options['ca_effect']) ? $options['ca_effect'] : 'slide',
        'space_between'       => isset($options['space_between']) ? intval($options['space_between']) : 20,
        'loop'                => $options['ca_loop'] === 'yes' ? 1 : 0,
        'speed'               => $options['ca_speed'],
        'autoplay'            => $options['ca_autoplay'] === 'yes' ? 1 : 0,
        'autoplay_delay'      => $options['ca_autoplay_delay'],
        'autoplay_hover'      => $options['ca_autoplay_hover'] === 'yes' ? 1 : 0,
        'navigation'          => $options['ca_navigation'] === 'yes' ? 1 : 0,
        'pagination'          => $options['ca_pagination'] === 'yes' ? 1 : 0,
        'dynamic_bullets'     => $options['ca_dynamic_bullets'] === 'yes' ? 1: 0,
        'pagination_type'     => $options['ca_pagination_type']
      );
	...
	public function gva_render_link_overlay($link = array(), $classes = 'link-overlay'){
		...
		if ( ! empty( $link['url'] ) ) {
		  $this->add_render_attribute( '_base_link_1' . $r, 'href', $link['url'] );
		  $this->add_render_attribute( '_base_link_1' . $r, 'aria-label', 'link' );
	...
	public function get_template($template_name = null){
		...
		if ( ! $template ){
			$template = GAVIAS_DONATM_PLUGIN_DIR  . 'elementor/views/' . $template_name;
		}
  	...
  	function donatm_get_template_part($slug, $name = null, $data = []){
		global $posts, $post;
  	...
  	public function pagination( $query = false ){
	 	...
	 	$translate['prev'] =  esc_html__('Prev page', 'donatm-themer');
	 	$translate['next'] =  esc_html__('Next page', 'donatm-themer');
	 	$translate['load-more'] = esc_html__('Load more', 'donatm-themer');
  	...
  	public function pagination_custom( $total = 1, $limit = 1 ){
	 	$paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : ( ( get_query_var( 'page' ) ) ? get_query_var( 'page' ) : 1 );
