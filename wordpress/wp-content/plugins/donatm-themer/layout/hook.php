...
class GVA_Layout_Frontend_Hook extends GVA_Layout_Model{
   ...
   function __construct(){
      add_action('donatm/layouts/page', array($this, 'layout_page'));
      
      add_action('donatm/layouts/single/listing', array($this, 'layout_single_listing'));

      add_action('donatm/layouts/single/post', array($this, 'layout_single_post'));
      add_action('donatm/layouts/archive/post', array($this, 'layout_archive_post'));

      add_action('donatm/layouts/single/template', array($this, 'single_template'));

      add_action('donatm/layouts/single/product', array($this, 'layout_single_product'));
      add_action('donatm/layouts/archive/product', array($this, 'layout_archive_product'));

      add_action('wp_enqueue_scripts', array($this, 'donatm_themer_elementor_load_css_header'), 500);
   }

   public function donatm_themer_elementor_load_css_header(){
      ...
      if($post && $post->post_type == 'job_listing'){
         ...
         $post_meta_template = get_post_meta($post->ID, 'donatm_template_layout', true);
   ...
   public function layout_page(){
      ...
      if($post_meta_template == '_without_layout' || empty($post_meta_template) || !metadata_exists('post', $post_id, 'donatm_template_layout')){
   ...
   public function layout_single_listing(){
      ...
      get_template_part('templates/listing/single');
   ...
   public function layout_single_product(){
      ...
      $post_id = $post->ID;
      $template_id = $this->get_template_default('single_product_layout');
      $post_template_id = get_post_meta($post_id, 'donatm_template_layout', true);
